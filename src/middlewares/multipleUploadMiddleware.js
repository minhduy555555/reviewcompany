...
let storage = multer.diskStorage({
  ...
  destination: (req, file, callback) => {
    callback(null, path.join(`${__dirname}/../public/img-video)`));
  },
  filename: (req, file, callback) => {
    ...
    let filename = `${Date.now()}-img-${file.originalname}`;
...
let uploadManyFiles = multer({storage: storage}).array("albums", 10);
